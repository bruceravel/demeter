...
** installation issues [0/1]
  - [ ] Relocation needs testing on Windows, need to make sure that
        the executables can all be found, also need to be able to find
        all of Ifeffit CL and CAPS files.

** Demeter bugs, shortcomings, missing features  [2/23] [8%]
...
** Windows issues [6/14] [42%]
  ...
  - [X] The Strawberry+Demeter package does not coexist at all with
        ...
        and somehow get gnuplot on the machine. *Solved by explicitly calling Strawberry in the .bat files*
...
** Artemis
*** Artemis bugs and missing features [9/18]  [50%]
   - [X] rename Atoms/Feff
   ...
   - [ ] importing dpj files is currently broken
