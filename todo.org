...
** Demeter bugs, shortcomings, missing features  [5/21] [23%]
  ...
  - [X] use plotkeys in single group plots, not in multiple group plots
...
*** needs doing [8/17] [47%]
   ...
   - [-] *PCA tool* [8/15]
       + [X] PCA in deriv(E)
       + [X] PCA in chi(k)
       + [X] save components to a file, put eigenvalues/eigenvectors in header
       + [X] save data stack to a file
       + [ ] manual entry of component coefficients (??)
       + [X] display coefficients of the TT
       + [X] save reconstruction to a file
       + [X] save target transform to a file
       + [ ] cluster analysis, select two components and plot the
             associations of each data with those two in a scatter plot.
       + [ ] persistance
       + [X] plot legend positioning for TT and recon plots needs to be sensitive to the space
...
** Windows issues [12/18] [66%] 					    :Windows:
  ...
  - [X] need a script for rewriting .bat files to redirect spew to log files.  this should run after installation as .bat's get overwritten
...
** Artemis
*** Artemis bugs and missing features [11/18]  [61%]
