...
** Demeter bugs, shortcomings, missing features  [4/23] [17%]
  ...
  - [X] Importing a Windows shortcut from a network mounted folder on linux or Mac will fail
  ...
  - [X] per data set R-factors, computed correctly for mkw fits (what is "correctly?")
  ...
  - [ ] Plucking from gnuplot really sucks
...
*** needs doing [10/22] [45%]
   ...
   - [-] *Difference tool* [3/4]
       + [X] plot diff for marked groups
       + [X] plot areas from marked groups
       + [X] make difference groups from marked groups
       + [ ] difference in k
   - [-] *LCF tool* [4/7]
...
** Windows issues [7/14] [50%]
  ...
  - [X] ScrolledWindow used in History plot tool does not scroll
  - [X] A link file linked to a directory does not get followed properly
...
** Artemis
*** Artemis bugs and missing features [12/18]  [66%]
   - [X] rename Atoms/Feff
   ...
   - [X] Restoring an historical fit does not sensibly preserve order,
	 fom numbering, etc
   - [X] Import project, need to fully import most recent fit so as to
	 populate log window correctly
   - [X] In GDS, all in type column after #13 added when restoring a
         fit are text rather than combobox
   - [X] force update of fit description when changing chi(k)
   - [X] importing dpj files is currently broken
   - [X] History plot tool is quite broken.  Need to make it work with
   ...
   - [X] per-data set R-factor reporting in log file is turned off.
   ...
   - [X] Andreas Voegelin's idea about mixed Feff/empicial standards
...
**** Project [3/7]
   ...
   - [X] does autosave file work as intended?
