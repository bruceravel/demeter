...
*** Athena
   ...
   - Fixed faulty output file from log-ratio analysis /{pre6}/


*** Artemis
   - Disable run atoms and run feff buttons after first DnD of a path
     (thanks to Yanyun Hu)
   - XDI related bug in Demeter::Fit (thanks to Allen Morris)
...
*** Demeter
   ...
   - Work around the mysterious situation on Windows wherein the MRU
     ...
     but changing from croak to carp when handling and unparsed line
     avoids it and issues a useful error message.  (thanks to Kurinji
     Krishnamoorthy)
   - XKCD-ish plots in A&A /{pre5}/
   - font picker control now available in Demeter::Wx::Config /{pre5}/
   ...
   - Fix a silly mistake introduced to one of the Chantler data files 
     when I did s{2014}{2015}g in January (thanks to Stevan Ognjanovic)
