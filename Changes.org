...
*** Artemis
   - read files.dat from old-style fit serialization so that ranking
     can be done
   - projects with quick first shell paths were not being imported
     properly
...
*** Windows
   - Implementing updater packages using Inno and wrapping upi the
     entire Demeter folder under {app}\perl\site\lib\
