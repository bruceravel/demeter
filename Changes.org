...
*** Athena
   ...
   - A change in 0.9.20 made it impossible to change the defaults for
     plotting ranges in the four spces.  This has been fixed.  (thanks
     to Lisa Bovenkamp) {/pre5/}
   - Fix buglet with ln checkbutton in col. sel. dialog reported as
     issue #22 (thanks to roetgenium) {/pre5/}
...
*** Artemis
   ...
   - Making VPaths and transfering paths now also plots in R
     ...
     Matt) {/pre4/}
   - plot after fit nw certain to respect k-weight setting in Plot
     window {/pre5/}
   - data+fit save file in Artemis are now XDI files {/pre5/}
   ...
   - Remove spurious minus sign appended to results of GDS evaluate
     button {/pre5/}
   - remove wxSHAPED attribute from GDS window's toolbar {/pre5/}
