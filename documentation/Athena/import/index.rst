...
data that most people use import into :demeter:`athena`.
...
In this example of a well formatted file, the header lines, the line
...
symbols, such as ``%`` or ``\*``, will also be understood as marking
header lines.
...
to their users, the truth is that :demeter:`athena` is widely used in
the XAS community.  A format that serves :demeter:`athena` well is
...
Your users would be well served by your adopting it.
