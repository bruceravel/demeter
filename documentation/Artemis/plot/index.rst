...
tab. Additional traces – the fit, the window, and so on – can be
...
“Other parameters” section at the bottom of the page. On the Path
...
:button:`Freeze,light` button below the plotting list. This will
...
There are several ways to do this, but the easiest is to select “Make
sum of all paths and plot in R” from the Actions menu on the Data page.
This will create `a VPath <plot/vpaths.html>`__ out of all the paths
included in the fitting model for the data. The VPath will be placed in
the plotting list in the `Plot window <plot/index.html>`__ and the plot
