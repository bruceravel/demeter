...
ok( ($groups[0] eq 'atoms' and $#groups == 20),                  'configuration system introspection works: groups '.$#groups);
my $groups = $demeter->co->main_groups;
ok( ($#{$groups} == 20),                                         'configuration system introspection works: main_groups '.$#groups);
...
ok( ($#groups == 21),                                            'introspection after reading new group: groups');
$groups = $demeter->co->main_groups;
ok( ($#{$groups} == 20),                                         'introspection after reading new group: main_groups');
