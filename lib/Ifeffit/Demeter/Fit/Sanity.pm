...

{
  ...
  my $functions = Ifeffit::Demeter->regexp('function');

  sub S_data_files_exist {
    my ($self, $r_problem) = @_;
    my @data = @{ $self->data };
    foreach my $d (@data) {
      next if $d->get("from_athena");
      my $file = $d->get("file");
      if (not -e $file) {
  ...
  sub S_defined_not_used {
    ...
    foreach my $g (@gds) {
      my $name = $g->name;
      my $found = 0;
      next if ($g->type ne 'guess');
      foreach my $d (@gds) {
	next if ($d->type !~ m{(?:def|restrain)});
  ...
  sub S_used_not_defined {
	...
	next if ($token =~ m{\A$functions\z});	    # function, ok
    ...
    foreach my $p (@paths) {
      ...
      my $label = $p->get('label');
      foreach my $pp (qw(s02 e0 delr sigma2 ei third fourth dphase)) {
	my @list = split(/$tokenizer_regexp+/, $p->get($pp));
	foreach my $token (@list) {
	  #print $mathexp, "  ", $token, $/;
	  next if ($token =~ m{\A\s*\z});	      # space, ok
	  next if ($token =~ m{\A$NUMBER\z});	      # number, ok
  ...
  sub S_parens_not_match {
    ...
    foreach my $g (@gds) {
      next if ($g->type =~ m{(?:merge|skip)});
      my $name = $g->name;
      my $not_ok = Tools->check_parens($g->mathexp);
      if ($not_ok) {
    ...
    foreach my $p (@paths) {
      ...
      foreach my $pp (qw(s02 e0 delr sigma2 ei third fourth dphase)) {
	my $mathexp = $p->get($pp);
	my $not_ok = Tools->check_parens($mathexp);
	if ($not_ok) {
  ...
  sub S_rmin_rbkg {
    ...
    my @data  = @{ $self->data  };
    foreach my $d (@data) {
      next if ($d->get('is_chi'));
      next if (not $d->get('fit_include'));
      if ($d->get('bft_rmin') < $d->get('bkg_rbkg')) {
	push (@{$$r_problem{errors}}, "Rmin is smaller than Rbkg for data set " . $d->get('label'));
	++$$r_problem{rmin_rbkg};
      };
    };
  };
  ...
  sub S_reff_rmax {
	...
	next if not defined($p);
	next if ($p->data ne $d);
	if ($p->get('reff') > ($d->get('bft_rmax')+1)) {
	  push (@{$$r_problem{errors}}, "Reff for path \"" . $p->get('label') . "\" is well beyond Rmax for data set \"" . $d->get('label') . "\"");
	  ++$$r_problem{reff_rmax};
	};
      };
    };
  ...
  sub S_exceed_ifeffit_limits {
    ...
    my $n_restraint = 0;
    foreach my $g (@gds) {
      ++$n_guess     if ($g->type eq 'guess');
      ++$n_params    if ($g->type !~ m{(?:merge|skip)});
      ++$n_restraint if ($g->type eq 'restrain');
    };
  ...
  sub S_program_var_names {
    my ($self, $r_problem) = @_;
    my @gds = @{ $self->gds   };
    my $regexp = Ifeffit::Demeter->regexp('program');
    foreach my $g (@gds) {
      if ($g->name =~ m{$regexp}) {
...
When the fit method is run, a series of sanity check on the data
