...
INF:        /inf/i
NAN:        /nan/i
FLOAT:      /[+-]?\ *(\d+(\.\d*)?|\.\d+)([eE][+-]?\d+)?/  # see perlretut
...
VERSION: COMM XDI_VERSION  APPLICATIONS(s?)
...
CRYSTAL:     COMM  "Crystal"            ":"  ELEMENT REFLECTION {
...
ENDTIME:     COMM  "End_time"           ":"  DATE TIME {
...
STARTTIME:   COMM  "Start_time"         ":"  DATE TIME {
	     $Xray::XDI::object->start_time(join(" ", $item[4], $item[5]));
            }
...
FIELD_NAME:      PROPERWORD
EXT_FIELD_NAME:  PROPERWORD

EXT_FIELD:  COMM  EXT_FIELD_NAME  ":"  ANY(s?)  {
...
COMMENT_LINE: NOTDASH  ANY(s?)  {
             $Xray::XDI::object->push_comment(join(" ", @{$item[2]}));
            }
COMMENTS:     COMMENT_LINE(s)  HEADER_END

LABEL:    PROPERWORD
LABELS:   COMM  PROPERWORD(s)  {
             $Xray::XDI::object->push_label(@{$item[2]});
            }

DATA_LINE: <skip: qr/[ \t]*/> FLOAT(s?) /[\n\r]/ {
             $Xray::XDI::object->add_data_point(@{$item[2]})  if $#{$item[2]}>-1;
