...
An overview of Demeter's file type plugins.
...
ill-formed numbers (e.g. NaN or missing whitespace between columns)
...
change the troubelsome data into a more easily handled from.
...
the data from the troublesome format into a standrad, well-formatted,
...
confuse Ifeffit and sometimes fail to have whitespace separating
numbers among the data columns.  The plugin comments out the headers
and corrects the problem with whitespace between columns.
...
As you can see, the plugins serve a couple of purposes.  One one hand,
...
data, as in the case of plugins which remove unnecessay columns from
the converted data files.

Plugins could be used for other sorts of data preprocessing.  For
example, it would be possible to apply deadtime corrections using
columns contained in the original data file.  A plugin could also be
used to remove a dark current from a data file which records the dark
current in the header but does not remove that dark signal from the
recored scalars.

=head2 Common attributes

Every file type plugin B<must> have the following attributes:
...
heavy-duty methematic utilities.
...
fluorescence XAS is predicatable, this method helps lower the barrier
of using the plugin.
...
"tranmission", then "fluorescence" is assumed.  (So spell carefully!)

If the columns are not predictable, then this method should return an
empty array.
...
An indiviual user can have additional plugins in user diskspace by
placing them in ...
