...
sub new {
  ...
  $hbox -> Add( Wx::StaticText->new($self, -1, "Name: "), 0, wxALL, 5);
  $self->{name} = Wx::TextCtrl->new($self, -1, q{});
  $hbox -> Add( $self->{name}, 1, wxGROW|wxALL, 10);
  $vbox -> Add( $hbox, 0, wxGROW|wxLEFT|wxRIGHT, 20 );

  $hbox = Wx::BoxSizer->new( wxHORIZONTAL );
  $hbox -> Add( Wx::StaticText->new($self, -1, "Distance: "), 0, wxALL, 5);
  $self->{reff} = Wx::TextCtrl->new($self, -1, q{3.0});
  $hbox -> Add( $self->{reff}, 1, wxGROW|wxALL, 10);
  $vbox -> Add( $hbox, 0, wxGROW|wxLEFT|wxRIGHT, 20 );
...
sub new {
  my $class = shift;
  my $this = $class->SUPER::new( @_[0..2], [300,50] );
  my $parent = $_[4];
