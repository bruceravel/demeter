...
Hephasetus -- A souped-up periodic table for the X-ray absorption spectroscopist
...
experiemntalist using a periodic table, tables of X-ray absorption
...
element in the periodic table, the elemnt name, symbol, and Z number
...
mavigate among the entries.  "Alt-e" and "Alt-h" post the "Hephaestus"
and "Help" menus.
...
The "data" table will be filld with some basic information
...
display a message in the statusbar giving the edge energy expressed in
...
llines associated with that edge in the "lines" table.
...
message in the statusbar giving the emission energy expressed in
wavelength units.
...
For element below Z=38, assume the K edge is being measured and use
the Z-1 element. 

=item 2

For element between Z=39 and 57, assume the K edge is being measured
...
formula must be stoichiometruic formula using a few simple rules.
...
Here is an example of the results printed for BN, with a denisty of
2.29 nad at energy of 9000 eV:
    ...
    cm requires 268.486 miligrams of sample at 9000.00 eV.
...
a given energy.  The calculation requires seevral parameters,
including
...
from a list of common lengtghs or supplied by the user.
...
update.  Clicking the "Reset" button retuns all the parameters to
their initial values.

As a rule of thumb, 10% is a good amount of absorption for the I0
chamber.  This will allow for a good meaurement of incidence flux
...
This tool displays a non-interactive chart explaining the tranisitions
...
measurments.  To search the list, enter an energy in the text box on
...
the incident beam and the exicitation of a much higher energy edge.
...
observed during measurments.  To search the list, enter an energy in
...
Anomalous scattering for elements can be plotted alone or overplotted
withother elements.  You can also select to plot either f', f", or
both.
...
The behavior of Hephaestus can be confugured via the preferences
...
given controls appoproiate to each parameter's data type for setting
...
branching ratios in one of the periodic table utilities, and providing
the Berger/Hubble XCOM tables and Feff's optical calculations as data
resources.
